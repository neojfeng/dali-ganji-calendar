...
3. 填好 `area`，用于页面筛选。当前建议值：`古城周边`、`喜洲周边`、`洱海北部`、`洱海东部`、`其他`。
4. 填好 `lat`、`lng`，页面会用它们在前端本地计算距离。
5. 选择合适的 `schedule_type` 并填写对应规则。
6. 运行 `python3 scripts/generate_events.py` 和 `python3 scripts/generate_static_calendars.py`。
...
落地页不会自动请求定位。用户点击“📍 使用当前位置”后，浏览器才会请求位置权限。
